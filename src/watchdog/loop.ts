import type { LobsterdConfig, TenantRegistry, TenantWatchState, Tenant } from '../types/index.js';
    ...
    try {
      for (const tenant of registry.tenants) {
        ...
        if (current.lastRepairAt) {
          ...
          if (elapsed < config.watchdog.repairCooldownMs) continue;
        }
        ...
        emitter.emit('check-complete', { tenant: tenant.name, results: checkResults });

        const oldState = current.state;
        const { next, needsRepair } = transition(current, checkResults, config.watchdog);
        tenantStates[tenant.name] = next;

        if (oldState !== next.state) {
          emitter.emit('state-change', { tenant: tenant.name, from: oldState, to: next.state });
        }
