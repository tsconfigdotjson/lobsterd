...
export function createOverlay(path: string, sizeMb: number): ResultAsync<void, LobsterError> {
...
export function resizeOverlay(path: string, newSizeMb: number): ResultAsync<void, LobsterError> {
  return exec(['truncate', '-s', `${newSizeMb}M`, path])
    .andThen(() => exec(['e2fsck', '-f', '-y', path]).orElse(() => exec(['true'])))
