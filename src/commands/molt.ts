...
export function runMolt(
  ...
  const progress = (tenant: string, phase: MoltProgress['phase'], detail?: string) =>
    onProgress?.({ tenant, phase, detail });

  return loadConfig().andThen((config) =>
    loadRegistry().andThen((registry): ResultAsync<MoltTenantResult[], LobsterError> => {
      let tenants: Tenant[];
      if (name) {
              ...
              .andThen((initialChecks): ResultAsync<MoltTenantResult, LobsterError> => {
                ...
                return runRepairs(tenant, failed, config).andThen((repairs) => {
                  ...
                  return runAllChecks(tenant, config).map((finalChecks) => ({
                    ...
                    repairs,
                    finalChecks,
                    healthy: finalChecks.every((c) => c.status === 'ok'),
                  }));
                });
              })
              .map((result) => {
                progress(tenant.name, 'done', result.healthy ? 'Healthy' : 'Still degraded');
