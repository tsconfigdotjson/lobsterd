...
function WatchApp({ tenants, handle }: {
  tenants: Tenant[];
  handle: ReturnType<typeof startWatchdog>;
}) {
  const [states, setStates] = useState<Record<string, TenantWatchState>>(handle.states());
  ...
  useEffect(() => {
    ...
    const unsub2 = handle.emitter.on('state-change', (data) => {
      setStates((prev) => ({ ...prev }));
    });
...
export async function runWatch(opts: { daemon?: boolean } = {}): Promise<number> {
  ...
  if (opts.daemon) {
    ...
    handle.emitter.on('state-change', (data) => {
      console.log(`[${new Date().toISOString()}] ${data.tenant}: ${data.from} → ${data.to}`);
    });

    handle.emitter.on('repair-complete', (data) => {
      for (const r of data.results) {
        console.log(`[${new Date().toISOString()}] ${data.tenant} repair ${r.repair}: ${r.fixed ? 'fixed' : 'failed'}`);
      }
    });
